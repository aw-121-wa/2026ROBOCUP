...
2. UART8 (PE0 RX/PE1 TX，115200、8N1) 接收厂家数据帧，校验长度、地址 0x20、状态和校验和后提取完整四字节 UID；128 字节有序接收环，溢出只记录 rfid_fault 位 64，不覆盖已有数据，也不停车。RFID 初始化和接收错误均单独诊断，不阻止握手或底盘任务。
...
读写器须使用厂家工具设为“自动读卡号、主动上传”，地址 0x20，115200 8N1。
