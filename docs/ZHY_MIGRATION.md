...
2. UART8 (PE0 RX/PE1 TX，9600、8N1) 接收厂家数据帧，校验长度、地址 0x20、状态和校验和后提取完整四字节 UID；128 字节有序接收环，溢出只记录 rfid_fault 位 64，不覆盖已有数据，也不停车。RFID 初始化和接收错误均单独诊断，不阻止握手或底盘任务。
...
5. App/path_chassis.c 单独承接后续底盘：转 178° → 后退 1810 mm → 红外靠桩/绕行 352° → 前进 330 mm → 台阶各段移动 → 左移 1650 mm → 仓库区域底盘移动。绕桩保留 5 秒靠桩、30 ms 红外稳定和 15 秒绕行保护；台阶保留对齐、-18 mm 起步、各点 -90 mm、段间 -117 mm。仓库区域只执行转 180°、对齐、左移 50 mm、后退 200 mm 三次、前进 200 mm 两次。
...
读写器 TX 接 PE0，RX 接 PE1，共地；接口电平须与板卡匹配。USART6 PC6/PC7 保留给步进电机。
