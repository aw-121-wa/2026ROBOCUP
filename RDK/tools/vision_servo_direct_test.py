...
7. After each completed G102 action, visual triggering is immediately armed again;
   there is no target-clear/rearm delay.
8. Stop normally after five completed G102 actions by default.
...
def main(argv=None) -> int:
    ...
    if not config.roi.x < trigger_x < roi_right:
        parser.error(
            f"--trigger-x must be inside ROI: {config.roi.x} < X < {roi_right}; "
            f"got {trigger_x}"
        )

    gate = RightToLeftDiscTrigger(
        roi_right=roi_right,
        trigger_x=trigger_x,
    ...
    board = HiwonderActionBoard(args.servo_port, args.servo_baud)
    ...
    try:
        ...
        while True:
            ...
            age_ms = (time.monotonic() - snapshot.timestamp) * 1000.0
            ...
            if result.reason != last_reason or result.valid or event is not None:
                print(
                    f"frame={snapshot.frame_id} state={gate.state} "
            ...
            print(
                f"{event} #{action_count} frame={snapshot.frame_id}; "
