 ...
 */
static inline uint32_t DWT_ElapsedCycles(uint32_t now,
                                         uint32_t last)
{
    return now - last;
}
 ...
 * 两个时间戳的秒数差
 */
static inline float DWT_DeltaSec(uint32_t now,
                                 uint32_t last)
{
...
{
    return (uint32_t)(
        ((uint64_t)SystemCoreClock * (uint64_t)us)
        / 1000000ULL
    );
}
...
{
    return (uint32_t)(
        ((uint64_t)SystemCoreClock * (uint64_t)ms)
        / 1000ULL
    );
}
